...
const allMethods = Object.values(methodCategories).reduce<MethodConfig>((acc, category: MethodCategory) => {
  return { ...acc, ...category.methods };
}, {});

// Convert methods to format needed for combobox
const methodOptions: MethodOption[] = Object.entries(allMethods).map(([value]) => ({
  value,
  label: value,
}));
        ...
        <header className="sticky top-0 z-50 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 px-4">
          <div className="flex h-14 items-center justify-between">
            ...
            <div className="flex items-center space-x-2">
              <Button variant="outline" onClick={addAllMethods}>Add All</Button>
              <Button variant="outline" onClick={removeAllCards}>Remove All</Button>
              <Button variant="outline" onClick={resetToDefault}>Default</Button>
              <span className="text-sm text-muted-foreground ml-4">
                Connected to: {import.meta.env.VITE_RPC_URL || "http://localhost:8545"}
              </span>
              <ModeToggle />
            </div>
          </div>
              ...
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                {cards.map((card) => (
                  <MethodCard
                    ...
                    onMethodSelect={(method) => updateCardMethod(card.id, method)}
